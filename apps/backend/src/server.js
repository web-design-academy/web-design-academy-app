...
const {packLessons, watchLessons} = require("./utils/pack");
const fs = require("fs");
...
async function bootstrap() {
  ...
  await packLessons(environment.lessonsPath, environment.cachePath).catch((error) => {
    console.log("Error processing lessons: ", error);
    process.exit(1);
  });

  watchLessons(environment.lessonsPath, environment.cachePath);
