...
function InspectorOverlay({ data }) {
  const { locale } = useChallenge();
  ...
  const marginStyle = {
    ...
    borderColor: colors.margin.bg, borderStyle: 'solid',
    borderTopWidth: `${mt}px`, borderRightWidth: `${mr}px`, borderBottomWidth: `${mb}px`, borderLeftWidth: `${ml}px`,
  };

  const borderStyle = {
    top: `${rect.top}px`, left: `${rect.left}px`,
    width: `${rect.width}px`, height: `${rect.height}px`,
    borderColor: colors.border.bg, borderStyle: 'solid',
    borderTopWidth: `${bt}px`, borderRightWidth: `${br}px`, borderBottomWidth: `${bb}px`, borderLeftWidth: `${bl}px`,
  };

  const paddingStyle = {
    top: `${rect.top + bt}px`, left: `${rect.left + bl}px`,
    width: `${rect.width - bl - br}px`, height: `${rect.height - bt - bb}px`,
    borderColor: colors.padding.bg, borderStyle: 'solid',
    borderTopWidth: `${pt}px`, borderRightWidth: `${pr}px`, borderBottomWidth: `${pb}px`, borderLeftWidth: `${pl}px`,
  };

  const contentStyle = {
    top: `${rect.top + bt + pt}px`, left: `${rect.left + bl + pl}px`,
    width: `${rect.width - bl - br - pl - pr}px`, height: `${rect.height - bt - bb - pt - pb}px`,
    display: layout?.type === 'grid' ? 'grid' : (layout?.type === 'flex' ? 'flex' : 'block'),
    ...
    backgroundColor: layout && (layout.rowGap > 0 || layout.columnGap > 0) ? colors.gridGap.bg : colors.content.bg
  };
  ...
  return (
    <div className="inspector-overlay-container">
      ...
      <div className="inspector-hud">
        <div className="tooltip-header" style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
          <div>
            <span className="tag-name">{tagName.toLowerCase()}</span>
            {className && <span className="class-name">.{className.split(' ').join('.')}</span>}
          </div>
          
          {layout && (
            <span style={{ background: '#db2777', color: 'white', padding: '2px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', marginLeft: '10px' }}>
        ...
        <div className="tooltip-body">
            {marginLabel && (
                <div className="legend-row">
                    <div className="legend-label">
                        <span className="color-swatch" style={{ backgroundColor: colors.margin.solid }}></span> margin
                    </div>
            ...
            {borderLabel && (
                <div className="legend-row">
                    <div className="legend-label">
                        <span className="color-swatch" style={{ backgroundColor: colors.border.solid }}></span> border
                    </div>
            ...
            {paddingLabel && (
                <div className="legend-row">
                    <div className="legend-label">
                        <span className="color-swatch" style={{ backgroundColor: colors.padding.solid }}></span> padding
                    </div>
            ...
            {layout && (layout.rowGap > 0 || layout.columnGap > 0) && (
                <div className="legend-row">
                    <div className="legend-label">
                        <span className="color-swatch" style={{ backgroundColor: colors.gridGap.solid }}></span> {t(locale, 'ui', 'gap')}
                    </div>
                    <span className="legend-val" style={{ color: '#ef4444' }}>
                        {layout.rowGap === layout.columnGap ? `${layout.rowGap}px` : `${layout.rowGap}px ${layout.columnGap}px`}
                    </span>
                </div>
            )}
            <div className="legend-row">
                <div className="legend-label">
                    <span className="color-swatch" style={{ backgroundColor: colors.content.solid }}></span> {t(locale, 'ui', 'dimensions')}
                </div>
