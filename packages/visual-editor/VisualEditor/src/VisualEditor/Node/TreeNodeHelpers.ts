...
export function normalizeTags(rawTags: any[]): TagDefinition[] {
  return rawTags.map(tag => ({
    ...
    attributes: (tag.attributes ?? []).map((attr: any) => ({
      name: attr.name,
      values: attr.values ? attr.values.flat().map((v: any) => String(v)) : undefined
    }))
  }));
}
...
export const updateTree = (items: Item[], targetId: string, updater: (i: Item) => Item): Item[] =>
  items.map(i => i.id === targetId
    ? updater(i)
