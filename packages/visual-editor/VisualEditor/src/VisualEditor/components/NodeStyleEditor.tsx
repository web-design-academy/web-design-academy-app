...
export default function NodeStyleEditor({ selector, cssContent, setCssContent, isDark }: StyleProps) {
  ...
  const renderInput = (prop: string, value: string, onChange: (v:string)=>void) => {
    if (COLOR_PROPS.includes(prop)) {
      return (
        <div className="flex items-center gap-1.5 ml-1.5" onClick={(e)=>e.stopPropagation()}>
          <input type="color" value={value.startsWith("#")?value:"#000000"} onChange={e=>onChange(e.target.value)} onClick={(e)=>e.stopPropagation()}  />
          <input value={value} onChange={e=>onChange(e.target.value)} className="px-1.5 py-1 text-black rounded" onClick={(e)=>e.stopPropagation()}  />
        </div>
      );
    }
    ...
    if (SIZE_PROPERTIES.includes(prop)) {
      ...
      return (
        <div className="flex items-center gap-1.5 ml-1.5" onClick={(e)=>e.stopPropagation()}>
          <input type="number" value={num} onChange={e=>onChange(e.target.value + unit)} className="w-20 px-1.5 py-1 text-black rounded" onClick={(e)=>e.stopPropagation()} />
          <select value={unit} onChange={e=>onChange(num + e.target.value)} className="px-1.5 py-1 rounded" onClick={(e)=>e.stopPropagation()}>
    ...
    return (
      <input value={value} onChange={e=>onChange(e.target.value)} className="ml-1.5 px-1.5 py-1 text-black rounded"  onClick={(e)=>e.stopPropagation()} />
    );
  };
  ...
  return (
    <div className={`flex flex-col gap-3 text-sm ${isDark ? "text-white" : "text-black"}`} onClick={(e)=>e.stopPropagation()}>
      ...
      {Object.entries(rules).map(([prop, value]) => (
        <div key={prop} className={`flex items-center gap-2 p-2 rounded border ${theme}`} onClick={(e)=>e.stopPropagation()} >
          <label className="flex-1 text-sm" onClick={(e)=>e.stopPropagation()} >
            {prop}:
      ...
      <div className={`border rounded overflow-hidden ${theme}`}>
        <input value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Found CSS" className={`w-full p-2 rounded outline-none border-2 border-white
                ${isDark
        ...
        <div className="max-h-56 overflow-y-auto mt-2">
          {ALL_PROPS
            ...
            .map(p => (
              <div key={p} onClick={()=>startAdd(p)}  className={`text-left px-2 py-1 rounded transition
                      ${
                        isDark
